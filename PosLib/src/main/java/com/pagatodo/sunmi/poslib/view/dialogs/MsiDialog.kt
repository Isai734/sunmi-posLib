...
class MsiDialog(private val listMonth: List<Int>, private val doContinue: (Int) -> Unit): DialogFragment(R.layout.fragment_msi_dialog), Serializable {

    private lateinit var binding: FragmentMsiDialogBinding
    private var monthSelected: Int = -1
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val finalListMonth = generateMonthList(listMonth,amount)

        binding.recycler.adapter = MsiAdapter(
            finalListMonth
        ) {month ->
            monthSelected = month.monthNumber
    ...
    companion object{
        fun create(totalAmount: String, msiList: List<Int>, doContinue: (Int) -> Unit): MsiDialog{
            val args = Bundle()
            args.putString("amount",totalAmount)
            var msiDialog = MsiDialog(msiList, doContinue)
