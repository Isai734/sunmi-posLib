...
abstract class SunmiTransaction {
    ...
    fun startPayProcess() {
        ...
        try {
            ...
            val parseLong = amount.toLong()
            if (parseLong > 0 || getTransactionData().transType == Constants.TransType.REFUND) {
                checkCard()
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    fun getPin(dataCard: DataCard) {
        PosLogger.e(TAG, "getPin (dataCard != null required): $dataCard")
        initPinPad(dataCard)
    }

    fun finishOnlineProcessStatus(tlvString: String? = null, tlvResponse: Constants.TlvResponses, message: String? = null) {//(CSU)Card Update Status
        try {
            ...
            PosLogger.e(TAG, "finishOnlineProcessStatus::card update status code::: $len")
            len?.also {
    ...
    private val mCheckCardCallback: CheckCardCallbackV2 = object : CheckCardCallbackV2Wrapper() {
        ...
        override fun onError(code: Int, message: String) {
            PosLogger.e(TAG, "onError: $code message $message")
    ...
    private val mEMVListener: EMVListenerV2 = object : EMVListenerV2.Stub() {
        ...
        override fun onWaitAppSelect(list: List<EMVCandidateV2>, b: Boolean) {
            ...
            PosLogger.e(TAG, "onWaitAppSelect b->$b candidateNames: $candidateNames")
            val appEmv = object : AppEmvSelectListener {
                override fun onAppEmvSelected(position: Int) {
                    try {
                        PosLogger.e(TAG, "onAppEmvSelected pos: $position")
        ...
        override fun onAppFinalSelect(appSelected: String) {
            PosLogger.e(TAG, "onAppFinalSelect tag9F06Value:$appSelected")
            if (appSelected.isNotEmpty()) {
                ...
                PosLogger.e(TAG, "detect $mAppSelect card")
            }
        ...
        override fun onConfirmCardNo(cardNo: String) {
            PosLogger.e(TAG, "onConfirmCardNo: $cardNo")
            mCardNo = cardNo
        ...
        override fun onRequestShowPinPad(pinType: Int, remainTime: Int) {
            PosLogger.e(TAG, "onRequestShowPinPad pinType: $pinType")
            mPinType = pinType
            isRequestPin = true
            initPinPad()
        }
        ...
        override fun onRequestSignature() {
            PosLogger.e(TAG, "onRequestSignature")
        ...
        override fun onCertVerify(p0: Int, p1: String?) {
            PosLogger.e(TAG, "onCertVerify p0-> $p0 p1-> $p1")
        }

        @Throws(RemoteException::class)
        override fun onCardDataExchangeComplete() {
            PosLogger.e(TAG, "onCardDataExchangeComplete")
        }

        @Throws(RemoteException::class)
        override fun onTransResult(code: Int, desc: String?) { //when has finalized process online
            PosLogger.e(TAG, "onTransResult code: $code desc: $desc")
            ...
            if (code == PosResult.CardAbsentAproved.code)
                checkAndRemoveCard()
            else
        ...
        override fun onRequestDataExchange(s: String) {
            PosLogger.e(TAG, "onRequestDataExchange s: $s")
        }
    }

    private val cardNoFromKernel: String
        //get card num from tags
        get() {
            try {
                ...
                posInstance().mEMVOptV2?.apply {
                    ...
                    if (len <= 0) {
                        PosLogger.e(TAG, "getCardNo error, code: $len")
                        return ""
                    }
                ...
                } ?: PosLogger.e(TAG, "EMVOptV2 is null")
    ...
    private fun setPayPassConfig(configId: String) {//Set tlv essential tlv data
        try {
    ...
    private val tlvData: Map<String, TLV>
        get() = try {
            val tagList = if (getTransactionData().tagsEmv.isNotEmpty())
                ...
                EmvUtil.tagsDefault
            PosLogger.e(TAG, "tagList to get-> ${tagList.toList()}")
            ...
            PosLogger.e(TAG, "tlvData:: " + exe.message)
            cancelOperation()
            emptyMap()
        }
            ...
            onShowPinPad(object : PinPadListenerV2.Stub() {
                ...
                override fun onConfirm(i: Int, pinBlock: ByteArray?) {
                    try {
                        ...
                        PosLogger.e(TAG, "initPinPad::onConfirm" + exe.message)
                        cancelOperation()
                    }
                }
                ...
                override fun onCancel() {
                    try {
                        ...
                        PosLogger.e(TAG, "initPinPad::onCancel" + exe.message)
                        cancelOperation()
                    }
                }
                ...
                override fun onError(code: Int) {
                    try {
                        PosLogger.e(TAG, "onError Pin:$code")
                        posInstance().mEMVOptV2?.importPinInputStatus(mPinType, 3)
                    } catch (exe: RemoteException) {
                        PosLogger.e(TAG, "initPinPad::onError" + exe.message)
                        cancelOperation()
                    }
                }
            }, pinPadConfig)
        } catch (exe: Exception) {
            PosLogger.e(TAG, "initPinPad::" + exe.message)
            cancelOperation()
        }
    }

    private fun checkAndRemoveCard() {//Check and notify remove card
        try {
            val status = posInstance().mReadCardOptV2?.getCardExistStatus(mCardType.value)?.also {
                if (it < 0) {
                    PosLogger.e(TAG, "getCardExistStatus error, code: $it")
                    onFailureTrx(PosResult.ErrorCheckPresentCard)
                    return
                }
            }
    ...
    companion object {
        val TAG: String = SunmiTransaction::class.java.simpleName
    }
